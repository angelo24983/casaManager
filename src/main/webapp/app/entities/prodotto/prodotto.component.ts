...
export class ProdottoComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.prodottoService
      ...
      .subscribe(
        (res: HttpResponse<IProdotto[]>) => this.paginateProdottos(res.body, res.headers),
  ...
  registerChangeInProdottos() {
    this.eventSubscriber = this.eventManager.subscribe('prodottoListModification', response => this.reset());
  }
  ...
  protected paginateProdottos(data: IProdotto[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.prodottos.push(data[i]);
    }
  }
